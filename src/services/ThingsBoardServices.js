...
export const getMovingHours = async (deviceId) => {
    try {
        ...
        const response = await axios.get(`https://prosthetic-api.onrender.com/thingsboard/movinghours/${deviceId}`, {
            headers: {
...
export const getSensorStatus = async (deviceId) => {
    try {
        const token = localStorage.getItem('auth_token');  // Obtener el token
        const response = await axios.get(`https://prosthetic-api.onrender.com/thingsboard/telemetry/${deviceId}`, {
            headers: {
                'Authorization': `Bearer ${token}`  // Pasar el token en el encabezado
            }
        });
