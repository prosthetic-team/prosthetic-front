...
const createPatient = async (token, patientData) => {
    try {
        const response = await axios.post(process.env.VUE_APP_API_URL + '/pacients', patientData, {
            headers: {
...
const getPatients = async () => {
    try {
        const response = await axios.get(process.env.VUE_APP_API_URL + '/pacients');
...
const getPatientById = async (id) => {
    try {
        const response = await axios.get(process.env.VUE_APP_API_URL + `/pacients/${id}`);
        return response.data;
    } catch (error) {
...
const getCompletedTreatments = async (token) => {
    try {
        const response = await axios.get(process.env.VUE_APP_API_URL + '/pacients/completed-treatments/all', {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        });
